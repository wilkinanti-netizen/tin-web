...
class _DriverHomeScreenState extends ConsumerState<DriverHomeScreen> {
  bool _isOnline = true;
  static const LatLng _center = LatLng(25.7617, -80.1918); // Miami, USA (Updated for US support)
      ...
      (position) {
        if (mounted) {
          ...
          if (_isOnline) {
            ...
            if (userId != null) {
              ref.read(profileRepositoryProvider).updateLocation(
                    userId,
                    position.latitude,
                    position.longitude,
  ...
  Widget _buildMainDriverContent(BuildContext context) {
    ...
    ref.listen<AsyncValue<List<Trip>>>(requestedTripsProvider, (
      ...
      if (nextCount > prevCount) {
        final firstTrip = next.asData?.value.first;
        if (firstTrip != null) {
