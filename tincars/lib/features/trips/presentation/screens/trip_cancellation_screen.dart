...
class _TripCancellationScreenState
  ...
  int _currentStep = 1; // Start directly at Reason step
  ...
  void _handleCancel() async {
    ...
    print(
      'TripCancellationScreen: [DEBUG] Iniciando proceso de cancelación para ${widget.trip.id}. Razón: $reason',
    );
    try {
      ...
      print(
        'TripCancellationScreen: [DEBUG] updateStatus completado con éxito.',
      );
    } catch (e) {
      print('TripCancellationScreen: [ERROR] Error durante updateStatus: $e');
    }

    // Force a fresh fetch of the active trip so HomeScreen doesn't see a stale "requested" state
    ref.invalidate(activeTripProvider);

    if (mounted) {
      // Regresar directo al Home para evitar la pantalla negra de doble pop
            ...
            itemBuilder: (context, index) {
              ...
              return ListTile(
                ...
                onTap: () {
                  ...
                  // If it's not 'Other', cancel immediately to save time
                  if (reason != l10n.reasonOther) {
                    _handleCancel();
                  }
                },
              );
