...
class _SearchingDriverScreenState extends ConsumerState<SearchingDriverScreen>
  ...
  void initState() {
    super.initState();
    print('SearchingDriverScreen: [LIFECYCLE] Initializing for Trip ID: ${widget.tripId}');
  ...
  @override
  void dispose() {
    print('SearchingDriverScreen: [LIFECYCLE] Disposing screen. Timer cancelled.');
  ...
  void _startTimeout() {
    _timeoutTimer = Timer.periodic(const Duration(seconds: 1), (t) {
      ...
      if (_secondsWaiting % 5 == 0) {
        print('SearchingDriverScreen: [HEARTBEAT] Still alive. Waiting for: $_secondsWaiting seconds.');
      }
      if (_secondsWaiting >= 480) {
        // 8 minutes timeout
        print('SearchingDriverScreen: [TIMEOUT] Reached 480 seconds limit.');
        t.cancel();
        _autoCancelNoDriver();
      }
    });
  ...
  Future<void> _autoCancelNoDriver() async {
    ...
    if (confirmed != false && mounted) {
        print('SearchingDriverScreen: Auto-cancelling trip due to timeout.');
        ref
  ...
  void _cancelTrip(Trip trip) async {
    ...
    if (result == true && mounted) {
      ...
      print('SearchingDriverScreen: Cancellation successful, screen already cleared by child.');
    }
  }
    ...
    ref.listen<AsyncValue<Trip>>(tripStreamProvider(widget.tripId), (
      ...
      if (trip != null) {
        ...
        if (trip.status != prevStatus) {
          print('SearchingDriverScreen: Trip status changed from $prevStatus to ${trip.status}');
        }
        ...
        if (trip.status == TripStatus.accepted ||
          ...
          _timeoutTimer?.cancel();
          print('SearchingDriverScreen: [NAVIGATION] Trip accepted/active. Status: ${trip.status}. Popping search screen to tracking.');
          ...
          _timeoutTimer?.cancel();
          print('SearchingDriverScreen: [DIAGNOSTICO] Trip cancelled detectado en DB.');
          
          if (_secondsWaiting < 5) {
             print('SearchingDriverScreen: [STABILITY] Cancelación ignorada (vía gracia de 5s). Segundos: $_secondsWaiting');
             return;
          }

          if (mounted && !_isClosing) {
            print('SearchingDriverScreen: [NAVIGATION] Popping screen DUE TO CANCELLED STATUS.');
            _isClosing = true; // Set immediately 
            setState(() {});
                      ...
                      child: SingleChildScrollView(
                        child: Column(
                          children: [
                            ...
                            const SizedBox(height: 12),
                            _buildAddressRow(
                              String.fromCharCode(66 + widget.intermediateStops.length),
                            ...
                            children: [
                              ...
                              if (widget.intermediateStops.isNotEmpty)
                                _buildStatItem(
                                  ...
                                  "\$${(widget.intermediateStops.length * (_pricingService.getPricingConfig(trip.vehicleType)['base'] as double)).toStringAsFixed(2)}", 
                                  "Parada extra"
                                ),
                            ],
                          ),
