-- Esquema de Base de Datos para TinCars (TINS CARS)
...
BEGIN 
    ...
    IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='trips' AND COLUMN_NAME='passenger_lat') THEN
        ALTER TABLE trips ADD COLUMN passenger_lat DECIMAL(10, 7);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='trips' AND COLUMN_NAME='passenger_lng') THEN
        ALTER TABLE trips ADD COLUMN passenger_lng DECIMAL(10, 7);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='trips' AND COLUMN_NAME='passenger_emoji') THEN
        ALTER TABLE trips ADD COLUMN passenger_emoji TEXT;
    END IF;
